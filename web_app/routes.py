...
class QuestionInput(BaseModel):
    question: str = Field(
        ...
        min_length=1,
        max_length=1000,
        description="The question to ask the learning system",
    )
    document_name: Optional[str] = Field(
        None,
        description=(
            "Name of document to query (without extension). "
            "If not provided, uses most recent."
        ),
    )
...
class AnswerOutput(BaseModel):
    ...
    expert_responses: Dict[str, str] = Field(
        description="Responses from each expert agent"
    )
...
def create_crew_for_document(document_name: str):
    ...
    if not store_path.exists():
        raise HTTPException(
            status_code=404, detail=f"Vector store for '{document_name}' not found"
        )
...
async def system_status():
    ...
    return SystemStatus(
        ...
        message=(
            f"System ready with {len(available_docs)} documents"
            if available_docs
    ...
    try:
        ...
        def ingest_uploaded_document(file_path: str, filename: str):
            ...
            try:
                ...
                documents = loader.load(
                    file_path
                )  # This now handles PDF/DOCX/TXT automatically
                print(f"📄 Loaded {len(documents)} document chunks")

                if not documents:
                    raise ValueError(
                        "No documents loaded - file may be empty or corrupted"
                    )

                valid_documents = [doc for doc in documents if doc.page_content.strip()]
                if not valid_documents:
                    raise ValueError(
                        "All document pages are empty after text extraction"
                    )
        ...
        return JSONResponse(
            content={
                ...
                "file_size": (
                    f"{file.size / 1024 / 1024:.2f} MB" if file.size else "Unknown"
                ),
                "status": "uploaded",
            }
        )
...
async def delete_document(filename: str):
    ...
    if not file_path.exists():
        raise HTTPException(
            status_code=404, detail=f"File '{filename}' not found in upload directory"
        )
    try:
...
async def delete_all_documents():
    ...
    try:
            ...
            for file_path in UPLOAD_DIR.iterdir():
                if file_path.is_file():
                    ...
                    files_deleted.append(
                        {"filename": file_path.name, "size_mb": round(file_size, 2)}
                    )
                    file_path.unlink()
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete all documents: {str(e)}"
        )
...
async def list_documents():
    ...
    try:
        ...
        return {
            ...
            "total_size_mb": round(total_size, 2),
            "available_for_query": [
                doc["document_name"] for doc in documents if doc["queryable"]
            ],
        }
    except Exception as e:
        raise HTTPException(
